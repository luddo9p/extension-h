    ...
    if (typeof chrome !== 'undefined' && chrome.cookies) {
      ...
      allCookies.forEach((cookie) => {
        chrome.cookies.remove(
          ...
          function (details) {
            details ? console.log(`Cookie ${cookie.name} supprimé.`) : console.error(`Erreur lors de la suppression du cookie ${cookie.name}.`);
          }
        );
      });
    } else {
...
async function resetCaches(e) {
  try {
    ...
    const itemsToRemove = [
      '1-last-update-time', '1-hapi-alliance-owned-planets', 'lastExecutionTimeForeign', 
      'connexion', 'prochaineExecution1', '1-hapiDataCache', 'lastExecutionTime', 
      'hapi-alliance-owned-planets', 'prochaineExecution2', 'hapiDataCacheMoves', 'hapiDataCache'
    ];
