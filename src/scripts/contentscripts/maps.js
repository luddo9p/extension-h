const setMap = async function () {
  ...
  if($('.ecomap').length > 0) {
    $('.ecomap').find('img').each( (i,el) => {
      $(el).attr('src', $(el).attr('src')+'?v=' + Math.floor(Math.random() * 1000000))
    })
  }
  ...
  localforage.getItem(gameId + '-alliance').then((store) => {
              ...
              $.each(planets, function (i, planet) {
                ...
                if (ally != '') {
                  ...
                  if (planet.govName == 'Hyp.') {
                    tr = $('<tr class="hyp">').attr('bgcolor', 'orange')
                  } else if (planet.govName == 'Dict.') {
                    tr = $('<tr class="dict">').attr('bgcolor', '#8a3333')
                  } else {
                    tr = $('<tr class="' + ally + '">')
                  }
                }
                // var find = store.data.find(
                //   (item) => item.planet === planet.name
                // )

                const owner = (find && find.player) ? '&nbsp;' + '' + find.player : ''
                const infos = owner
                console.log(infos)
                var td = $('<td>').append([
                  $(
